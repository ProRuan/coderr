...
def get_order_count_url(business_id):
    """
    Get URL of order count.
    """
    return reverse('order-count', args=[business_id])


class OpenOrderCountTests(APITestCase):
    """
    Tests for retrieving the amount of orders 'in progress'.
    """
    ...
    def test_get_order_count_success(self):
        """
        Ensure authenticated user gets amount 
        of orders in progress (HTTP 200).
        """
        self.client.force_authenticate(self.customer)
        url = get_order_count_url(self.business.id)
    ...
    def test_get_order_count_business_not_found(self):
        """
        Ensure non-existent business user returns HTTP 404.
        """
        self.client.force_authenticate(self.customer)
        url = get_order_count_url(9999)
        res = self.client.get(url)
